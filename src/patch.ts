...
export async function patch(options: PatchOptions): Promise<object> {
  ...
  const pathPointer=JSONPath({path: yamlPath, json: document, resultType: 'pointer'})[0];

  if (yamlInsideYamlPath) {
    // This does the same as the pointer, but on a format that jsonpath likes...
    const path=JSONPath({path: yamlPath, json: document, resultType: 'path'})[0];
    ...
    const pathInsideYaml = JSONPath({path: yamlInsideYamlPath, json: yamlInsideYaml, resultType: 'pointer'})[0];
